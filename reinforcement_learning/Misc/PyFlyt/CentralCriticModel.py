...
class CentralStackedModel(TorchModelV2, nn.Module):
    ...
    def __init__(
        ...
        if self.num_frames > 1:
            self.action_stack_dims = action_space.shape[0] * self.num_frames * self.num_agents
        ...
        self.view_requirements['all_actions'] = ViewRequirement(
            data_col = 'actions',
            space = action_space,
            shift = "-{}:0".format(self.num_frames),
            used_for_compute_actions = True,
            used_for_training = True
        )
...
ModelCatalog.register_custom_model("CentralStackedModel", CentralStackedModel)