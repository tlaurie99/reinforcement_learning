...
def centralized_critic_postprocessing(policy, sample_batch, config, other_agent_batches = None, episode = None):
    ...
    if policy.loss_initialized():        
        ...
        if global_state_flag:
            ...
            for agent_name in agent_names:
                if agent_name in other_agent_batches:
                    ...
                    state_batch_list.append(opponent_batch[index]['state'])
                    next_immediate_obs.append(opponent_batch[index]['new_obs'])
                    next_state_batch_list.append(opponent_batch[index]['next_state'])
                else:
                    state_batch_list.append(sample_batch['state'])
                    next_immediate_obs.append(sample_batch['new_obs'])
                    next_state_batch_list.append(sample_batch['next_state'])
            #stack observations along the num_agent dimension
            observations = np.stack(state_batch_list, 1)
            next_immediate_obs = np.stack(next_immediate_obs, 1) 
            next_observations = np.stack(next_state_batch_list, 1)
            #reshape observations to concatenate along the observation dimension
            next_immediate_obs = next_immediate_obs.reshape(next_immediate_obs.shape[0], -1)
            sample_batch['ObsFrameStack'] = observations.reshape(observations.shape[0], -1)
            next_observations_flat = next_observations.reshape(next_observations.shape[0], -1)
            #concatenate next observations with immediate next observation (i.e.: t = -3, t = -2, t = -1, t = 0, concatenated t = 1)
            sample_batch['ObsFrameStackNextState'] = np.concatenate((next_observations_flat, next_immediate_obs), axis = -1)

        #now use the central value function to get the MoG predictions
        sample_batch[SampleBatch.VF_PREDS] = policy.compute_central_vf((convert_to_torch_tensor(sample_batch['ObsFrameStack'], 
        ...
        sample_batch['state'] = np.zeros((o.shape[0], o.shape[-1]), dtype = sample_batch[SampleBatch.CUR_OBS].dtype)
        sample_batch['next_state'] = np.zeros((n_o.shape[0], n_o.shape[-1]), dtype = sample_batch[SampleBatch.NEXT_OBS].dtype) 
        sample_batch['ObsFrameStack'] = np.zeros((o.shape[0], o.shape[-1] * num_agents * num_frames), dtype = sample_batch[SampleBatch.CUR_OBS].dtype)
        sample_batch['ObsFrameStackNextState'] = np.zeros((n_o.shape[0], n_o.shape[-1] * num_agents * num_frames), dtype = 
...
class SameStructurePolicy(ValueNetworkMixin, LearningRateSchedule, EntropyCoeffSchedule, KLCoeffMixin, TorchPolicyV2):
    ...
    def updating_view_reqs(self, observation_space, action_space, config):
        ...
        self.view_requirements['ObsFrameStack'] = ViewRequirement(
            data_col = 'state',
            space = observation_space,
            shift = 0,
            used_for_compute_actions = True,
        ...
        #this would not work with new_obs below, so I had to use the offset of num_frames - 2 and concatenate the current states' new_obs
        # obs:      (t = -4, t = -3, t = -2, t = -1, t = 0)
        # next_obs:         (t = -3, t = -2, t = -1, t = 0, concatenated t = 1)
        # the concatenation is done in the centralized_critic_postprocessing function
        self.view_requirements['next_state'] = ViewRequirement(
            data_col = 'obs',
            space = observation_space,
            shift = "-{}:0".format(num_frames - 2),
            used_for_training = True
        )
        self.view_requirements['ObsFrameStackNextState'] = ViewRequirement(
            data_col = 'next_state',
            space = observation_space,
            shift = 0,
            used_for_compute_actions = True,
